...
from dataset_final import DATASET_CFG, prepare_yolo_dataset
from metrics_95 import (
...
def _compact_epoch_line(trainer):
    ...
    try:
        ...
        met = getattr(v, "metrics", None)
        maps = getattr(met, "maps", None)  # [0.50,0.55,...,0.95] 共10个阈值
        if maps is not None and len(maps) >= 10:
            maps = np.array(maps, dtype=float)
            map75 = float(maps[5])   # 0.75 对应第6个
            map95 = float(maps[9])   # 0.95 对应第10个
    except Exception:
        pass
    # 记录到历史
    epoch_record = {
    ...
    line = (f"E{e:03d}/{E:03d} | bs={bs:<3d} | sz={imgsz:<4d} "
            ...
            f"| mAP75={(map75 if not np.isnan(map75) else float('nan')):.3f} "
            f"| mAP95={(map95 if not np.isnan(map95) else float('nan')):.4f} "
...
def _on_train_end(trainer):
    """训练正常结束时的回调"""
    try:
        save_dir = Path(trainer.save_dir) if hasattr(trainer, 'save_dir') else _CURRENT.get("save_dir")
        if not save_dir:
            _root_logger.warning("[TRAIN] 无法获取 save_dir，跳过保存")
            return
            
        final_epoch = int(trainer.epoch) + 1 if hasattr(trainer, 'epoch') else 0
...
def main():
    ...
    for exp in CFG["experiments"]:
        ...
        if results is not None and hasattr(results, 'save_dir'):
            ...
            _root_logger.info(f"✓ 从 results 获取 save_dir：{save_dir}")

        # 方法2：从 model.trainer 获取
        if save_dir is None:
            trainer_obj = getattr(model, "trainer", None)
            if trainer_obj and hasattr(trainer_obj, 'save_dir'):
                save_dir = Path(trainer_obj.save_dir)
                _root_logger.info(f"✓ 从 model.trainer 获取 save_dir：{save_dir}")
        ...
        if save_dir is None:
            ...
            if save_dir:
                ...
                _root_logger.info(f"✓ 从 _CURRENT 获取 save_dir：{save_dir}")

        # 方法4：搜索项目目录
        if save_dir is None:
            project_dir = Path(CFG["train"]["project"])
            possible_dirs = list(project_dir.glob(f"{exp_name}*"))
            if possible_dirs:
                save_dir = max(possible_dirs, key=lambda p: p.stat().st_mtime)
                _root_logger.info(f"⚠ 搜索到可能的 save_dir：{save_dir}")

        # 方法5：使用预期目录
        if save_dir is None:
            save_dir = expected_dir
            _root_logger.warning(f"⚠ 使用预期目录作为 save_dir：{save_dir}")

        # 最终确认
        _CURRENT["save_dir"] = save_dir
        _root_logger.info(f"[FINAL] save_dir = {save_dir}")

        # ========== 步骤2：移动日志文件 ==========
        final_console_log = save_dir / "train_console.log"
        try:
            if tmp_console_log.exists():
                final_console_log.parent.mkdir(parents=True, exist_ok=True)
                if final_console_log.exists():
                    final_console_log.unlink()
                shutil.move(str(tmp_console_log), str(final_console_log))
                _root_logger.info(f"✓ 日志已移动：{final_console_log}")
        except Exception as e:
            _root_logger.warning(f"⚠ 移动日志失败：{e}")
            try:
                if tmp_console_log.exists():
        ...
        if not weights_dir.exists():
            _root_logger.error(f"✗ weights 目录不存在：{weights_dir}")
            _root_logger.error(f"[DEBUG] save_dir 内容：")
            try:
                for item in save_dir.iterdir():
                    _root_logger.error(f"  - {item.name}")
            except Exception as e:
                _root_logger.error(f"  无法列出目录：{e}")
            _root_logger.error(f"跳过此实验")
            _remove_handler(_root_logger, exp_file_handler)
            continue

        # 列出 weights 目录内容
        _root_logger.info(f"[DEBUG] weights 目录内容：")
        available_weights = []
        try:
            available_weights = list(weights_dir.glob("*.pt")) + list(weights_dir.glob("*.pth"))
            for w in available_weights:
                _root_logger.info(f"  - {w.name} ({w.stat().st_size / 1024 / 1024:.2f} MB)")
        except Exception as e:
            _root_logger.error(f"列出 weights 失败：{e}")

        if not available_weights:
            _root_logger.error(f"✗ weights 目录为空")
            _root_logger.error(f"跳过此实验")
            _remove_handler(_root_logger, exp_file_handler)
            continue
        ...
        weight_to_use = None
        weight_candidates = [
            (weights_dir / "best_fitness.pt", "自定义最佳fitness模型"),
            (weights_dir / "best.pt", "Ultralytics最佳模型"),
            (weights_dir / "last.pt", "最后epoch模型"),
        ]

        for weight_path, desc in weight_candidates:
            if weight_path.exists():
                weight_to_use = str(weight_path)
                _root_logger.info(f"✓ 选择权重：{desc} ({weight_path.name})")
                break

        if weight_to_use is None:
            _root_logger.error(f"✗ 未找到任何可用的权重文件 (best.pt/last.pt)")
            _root_logger.error(f"跳过此实验")
            _remove_handler(_root_logger, exp_file_handler)
            continue
        ...
        with tee_stdout_stderr(final_console_log):
            try:
                ...
                _root_logger.info(f"✓ 验证完成")
            except Exception as e:
                _root_logger.error(f"✗ 验证失败：{e}\n{traceback.format_exc()}")

        if val_res is None:
            _root_logger.error(f"✗ 验证返回 None，跳过此实验")
            _remove_handler(_root_logger, exp_file_handler)
            continue
        ...
        try:
            ...
            _root_logger.info(f"✓ 指标提取：mAP50={m['map50']:.4f}, mAP75={m['map75']:.4f}, mAP95={m['map95']:.6f}, mAP50-95={m['map5095']:.4f}")
        except Exception as e:
            _root_logger.error(f"✗ 指标提取失败：{e}")
            m = {
                "map50": float("nan"),
                "map75": float("nan"),
                "map95": float("nan"),
        ...
        try:
            size_mb = file_size_mb(weight_to_use)
        except Exception:
            size_mb = 0.0
        ...
        with tee_stdout_stderr(final_console_log):
            try:
                ...
                _root_logger.info(f"✓ 时延统计：mean={latency['latency_mean_ms']:.2f}ms, p95={latency['latency_p95_ms']:.2f}ms")
            except Exception as e:
                _root_logger.error(f"✗ 时延统计失败：{e}")
                latency = {
            ...
            try:
                ...
                with tee_stdout_stderr(final_console_log):
                    model_best.predict(
                        ...
                        device=eval_device,
                        save=True, 
                        conf=CFG["predict_vis"]["conf"],
                        project=str(save_dir), 
                        name="_pred_vis", 
                        exist_ok=True, 
                        verbose=False
                    )
                _root_logger.info(f"✓ 预测可视化：{save_dir / '_pred_vis'}")
            except Exception as e:
                _root_logger.warning(f"⚠ 预测可视化失败：{e}")
        ...
        summary = {
            ...
            "artifacts": {
                ...
                "plots_dir": plots_dir,
                "best_pt": weight_to_use,
                "last_pt": str(weights_dir / "last.pt") if (weights_dir / "last.pt").exists() else "N/A",
        ...
        try:
            ...
            _root_logger.error(f"✗ 保存 summary 失败：{e}")
        ...
        try:
            ...
            _root_logger.warning(f"⚠ 保存 LaTeX 行失败：{e}")
            latex_row = "N/A"
        ...
        global_rows.append({
            ...
            "mAP@0.75": summary["metrics"]["mAP@0.75"],
            "mAP@0.95": summary["metrics"]["mAP@0.95"],
